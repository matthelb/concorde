...
/****************************************************************************/
/*                                                                          */
/*        CHAINED LIN-KERNIGHAN  -  Modified for Hamiltonian Path           */
/*                                                                          */
/*                           TSP CODE                                       */
/*                                                                          */
/*                                                                          */
...
/*    EXPORTED FUNCTIONS:                                                   */
/*                                                                          */
/*  int CClinkern_path (int ncount, CCdatagroup *dat, int ecount,           */
/*      int *elist, int nkicks, int *inpath, int *outpath,                  */
/*      double *val, int silent, CCrandstate *rstate)                       */
/*    RUNS Chained Lin-Kernighan, with the edge from the start node to the  */
/*      end node fixed (so it looks for a Hamiltonian path from the start   */
/*      to the end node)                                                    */
...
/*    -inpath  (a starting Hamiltonian path, in node node node format -     */
/*              can be NULL)                                                */
/*    -outpath (returns the Hamiltonian path - can be NULL)                 */
/*    -run_slightly (if nonzero, then very little info will be printed)     */
/*                                                                          */
/*    NOTES: If inpath is NULL, then the linear ordering 0, ..., n-1 is     */
/*     used, and the edge 0->n-1 is fixed.                                  */
/*                                                                          */
/****************************************************************************/
...
#define Fixededge(n1,n2) (((n1 == G->s && n2 == G->t) ||                \
                           (n2 == G->s && n1 == G->t)) ? 1 : 0)
...
static int
   ...
   check_path (graph *G, int *path),
   load_path (graph *G, int *path, int *outpath);

static double
   improve_tour (graph *G, distobj *D, adddel *E, aqueue *Q, CClk_flipper *F,
       int start, flipstack *fstack, CCptrworld *intptr_world,
       CCptrworld *edgelook_world),
   path_length (graph *G, int ncount, int *cyc, distobj *D);
...
int CClinkern_path (int ncount, CCdatagroup *dat, int ecount, int *elist,
        int nkicks, int *inpath, int *outpath, double *val, int silent,
        CCrandstate *rstate)
{
    int rval = 0;
    int i;
    int *tcyc = (int *) NULL;
    double startzeit, tval;
    graph G;
    distobj D;
    CCptrworld intptr_world;
    CCptrworld edgelook_world;
    ...
    if (inpath) {
        for (i = 0; i < ncount; i++) tcyc[i] = inpath[i];
    } else {
        for (i = 0; i < ncount; i++) tcyc[i] = i;
    }
    G.s = tcyc[0];
    G.t = tcyc[ncount-1];
    *val = path_length (&G, ncount, tcyc, &D);
    if (silent == 0) {
        printf ("Starting Path: %.0f\n", *val); fflush (stdout);
    }
    ...
    rval = check_path (&G, tcyc);
    CCcheck_rval (rval, "ERROR: Fixed edge was lost");

    if (outpath) {
        rval = load_path (&G, tcyc, outpath);
        CCcheck_rval (rval, "load_path failed");
 
        tval = path_length (&G, ncount, outpath, &D);
        if (tval != *val) {
            fprintf (stderr, "ERROR: Arrange path has wrong length\n");
            rval = 1;  goto CLEANUP;
        }
    }
...
static double path_length (graph *G, int ncount, int *cyc, distobj *D)
{
    int i;
    double val = 0.0;
...
static int check_path (graph *G, int *path)
{
    int i;

    for (i = 1; i < G->ncount; i++) {
        if (Fixededge (path[i-1], path[i])) return 0;
    }
    if (Fixededge (path[0], path[G->ncount-1])) return 0;

    return 1;
}

static int load_path (graph *G, int *path, int *outpath)
{
    int i, k, sind = 0, tind = 0;
    int ncount = G->ncount;
    int s = G->s;
    int t = G->t;
    ...
    for (i = 0; i < ncount; i++) {
        if (path[i] == s)  {
            sind = i;
            if (i == 0) {
                if (path[ncount-1] == t) tind = ncount-1;
                else if (path[i+1] == t) tind = i+1;
                else return 1;
            } else if (i == ncount-1) {
                if (path[i-1] == t) tind = i-1;
                else if (path[0] == t) tind = 0;
                else return 1;
            } else {
                if (path[i-1] == t) tind = i-1;
                else if (path[i+1] == t) tind = i+1;
    ...
    k = 0;
    if (tind == sind+1 || (tind == 0 && sind == ncount-1)) {
        for (i = sind; i >= 0; i--) outpath[k++] = path[i];
        if (tind > 0) {
            for (i = ncount-1; i >= tind; i--) outpath[k++] = path[i];
        } 
    } else {
        for (i = sind; i < ncount; i++) outpath[k++] = path[i];
        if (sind > 0) {
            for (i = 0; i <= tind; i++) outpath[k++] = path[i];
        }
    }
    if (k != ncount) {
        fprintf (stderr, "Did not gather the path correctly\n");
        return 1;
    }
